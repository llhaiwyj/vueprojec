<template>
	<div class="conmmercial">		
					...
					<el-row :gutter="10">
						...
						<el-col class="row" :xs="19" :sm="18" :md="11" :lg="10" :xl="5">
						 ...
						 <template class="biaoti" slot="prepend">目前状态</template>
							...
							<div class="input-group">
								<span class="input-group-addon">商户使用状态</span>
		...
		<modal v-show="isModalVisible" @close="closeModal" v-on:closes="serlf" v-on:close="serlfs" v-bind:accountlist="accounts"></modal>
		<modals v-show="isModalVisibles" @close="closeModals" v-on:closes="serlf" v-on:close="serlfs" v-bind:accountlist="accounts" v-bind:cityArr="chengshi" v-bind:districtArr="xianqu" v-bind:usinfor="userinformation"></modals>
		<renewalfee v-show="isrenewalfee" v-bind:renewalfeeArrs="renewalfeeArr" v-on:close="serlf"></renewalfee>
	</div>
</template>

<script>
	import Modal from './merchmotai.vue'
	import Modals from './merchmodify.vue'
...
//				console.log(this.merchantData[index])
			...
			deletes(index, user) {
				this.$confirm('此操作将永久删除该文件, 是否继续?', '提示', { confirmButtonText: '确定', cancelButtonText: '取消', type: 'warning' }).then(() => {
					this.$ajax.post(`${this.$url}/bussiness/valDelBussiness.html`, { ids: user[index].bussinessID }).then(data => {
						console.log(data)
						if(data.data.flag == true) {
							this.$message({
								type: 'success',
			...
			showModals: function(index, all) {
				...
				this.$ajax.post(`${this.$url}/bussiness/initUpdateBussinessList.html`, { bussinessId: all[index].bussinessID }).then(data => {
						console.log(data)
						this.userinformation = (data.data.data.list[0])
						
                        this.chengshi=[]
						for(let i in this.listCity) {
							if(this.listCity[i].parentID == data.data.data.list[0].provinceID) {
								this.chengshi.push(this.listCity[i])
							}
						}
						this.xianqu=[]
						for(let i in this.listCity) {
							if(this.listCity[i].parentID == data.data.data.list[0].cityID) {
