			...
			<el-table ref="multipleTable" :data="storemanage" height="632" tooltip-effect="dark" style="width: 100%">
				...
				<el-table-column prop="state" label="服务状态 " show-overflow-tooltip>
					<template slot-scope="scope">
						<el-button id="zi" type="text" v-if="scope.row.state==1" size="small">启用</el-button>
						<el-button id="zi1" type="text" v-else="scope.row.state==0" size="small">停用</el-button>
					</template>
				</el-table-column>
...
//				    isShowDel:false,
		...
		methods: {
			...
			showModals(index, all) {
				...
				this.$ajax.post(`${this.$url}/storeService/initStoreServiceUpdate.html`, { serviceID: all[index].serviceID }).then(data => {
						console.log(data)
