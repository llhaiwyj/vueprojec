<template>
	<div class="rolesettings">
		...
		<modals v-show="isModalVisibles"@close="closeModals" v-bind:uploadfil="uploadfil" v-bind:sels="sel" v-bind:arrs="arr" v-bind:bussarrs="bussarr" v-bind:employee="employe" v-bind:dataId="dataid" v-bind:roleID="roleId" v-bind:bussdataId="bussdataid" v-on:close="serlf"/>
	</div>
</template>
...
//             console.log(all[inde].roleID)
               this.roleId=all[inde].roleID
               let state=all[inde].state
...
//						this.sel=[{ value: 1, text: '系统管理员' },{ value: 2, text: '商户管理员' },{ value: 3, text: '门店管理员' },]
               ...
               //获取回显的数据
               this.$ajax.post(`${this.$url}/employee/loadAddTree.html`, {state:state }).then(res => {
						console.log(res)
