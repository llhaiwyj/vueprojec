<template>
  <div class="city-select">
    ...
    <select placeholder="所在县" v-on:change="xianqu($event)">
      <option v-for="item in fourcity" :value="item.businessName">{{item.businessName}}</option>
    </select>
   ...
   export default {
    ...
    watch: {
      ...
      selectedBlock() {
        Vue.nextTick(() => {
          // this.$emit('input', this.info);
          // 得到前三级的信息
          this.fourcity = this.info;
          this.Allcity = this.info;
          this.$ajax.post(`${this.$url}/four_level/selAreaList.html`,{parentID:this.fourcity.block.code}).then(data => {
    ...
    methods:{
      xianqu(event){
          ...
          let newarr = [];
          newarr.push(this.Allcity.province.name);
          newarr.push(this.Allcity.city.name);
          newarr.push(this.Allcity.block.name);
          newarr.push(event.target.value);
          newarr.join('');
          console.log(newarr);
          this.$emit('fatherCommon',newarr);
